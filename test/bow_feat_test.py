...
class BoWFeatureTest(unittest.TestCase):
    ...
    def test_bag_words2vec_unigram(self):
        ...
        sample_corpus = feat.corpus_list(self.sample_text)
        mat_feat = feat.bag_words2vec_unigram(sample_corpus, self.sample_text)
        assert len(sample_corpus) == len(mat_feat[random.randrange(0, 2)])
        feat.visual_unigram(sample_corpus, mat_feat)

    def test_bag_words2vec_unigram_real(self):
        # take only 10 reviews into test part
        texts = text.read_data_tag_from_file('neg')[:10]
        sample_corpus = feat.corpus_list(texts)
        mat_feat = feat.bag_words2vec_unigram(sample_corpus, texts)
        assert len(sample_corpus) == len(mat_feat[random.randrange(0, 10)])
        feat.visual_unigram(sample_corpus, mat_feat)
